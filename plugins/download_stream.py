...
async def download_video(client, CallbackQuery):
    ...
    try:
        await client.send_video(
            ...
            progress_args=(
                pablo,
                f"**مرحبا صديقي {chutiya} ساقوم بتحميل الفيديو وارساله لك انتضر فضلا.**",
                file_stark,
            ),
        )
...
async def download_audio(client, CallbackQuery):
    ...
    pablo = await client.send_message(
        CallbackQuery.message.chat.id,
        f"**مرحبا صديقي {chutiya} ساقوم بتحميل الاغنية وارسالها انتضر فضلا.**",
    )
    if not videoid:
        await pablo.edit(
            f"**صديقي {chutiya} لم اجدها علئ اليوتيوب انا اسف •**"
    ...
    capy = f"❄ **الرابط :** [{thum}]({mo})\n\n💫 **القناة :** {thums}\n\n🥀 **هذا من اجلك :** {chutiya}\n\n⏳ **المدة الزمنية :** {int(ytdl_data['duration']) // 60}:{int(ytdl_data['duration']) % 60}"
    try:
